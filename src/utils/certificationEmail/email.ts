...
async function generateCertificate(
  ...
  try {
    ...
    if (!fs.existsSync(templatePath)) {
      console.log(
        `Template file '${certificateType}.html' not found at '${templatePath}'`,
      );
      throw new Error(
        `Template file '${certificateType}.html' not found at '${templatePath}'`,
      );
...
async function sendCertificate(
  ...
  try {
    console.log(
      `Sending ${certificateType} certificate to ${participantEmail} for ${eventName}...`,
    );
    ...
    console.error(
      `Failed to send ${certificateType} certificate to ${participantEmail}:`,
      error,
    );
    throw error;
  }
